...
export class Jsonx implements Iterable<Jsonx> {
    ...
    public static parse(xml: string) {
        return new Jsonx(toJson(xml));
    }

    /** 原始資料。 */
    public readonly data: xj.ElementCompact | xj.ElementCompact[];

    /**
     * 建立物件從 xml 字串或是 ElementCompact。
     */
    constructor(content: string | xj.ElementCompact | xj.ElementCompact[] = {}) {
        if (typeof(content) === 'string') {
            this.data = toJson(content);
        } else {
            this.data = content;
        }
    }
    ...
    public set text(val: string) {
        this.checkNotArray();
        (this.data as xj.ElementCompact)._text = val;
    }

    /** 讀取或寫入 CDATASection 資料。 */
    public get cdata() {
        this.checkNotArray();
        return (this.data as xj.ElementCompact)._cdata;
    }

    public set cdata(val: string) {
        this.checkNotArray();
        (this.data as xj.ElementCompact)._cdata = val;
    }
    ...
    public setAttr(name: string, val: string) {
        ...
        const data = this.data as xj.ElementCompact
    ...
    public child(...name: string[]) {
        let parent: xj.ElementCompact = this.data;
        let child: Jsonx;
     ...
     * @param {(string | Jsonx | xj.ElementCompact)} content Xml 字串、Jsonx 物件、一般物件。
     * @memberof Jsonx
     */
    public raw(name: string, content: string | Jsonx | xj.ElementCompact | xj.ElementCompact[]) {

        if(typeof(content) === 'string') {
            this.data[name] = toJson(content);
    ...
    private getChild(childName: string, parent: xj.ElementCompact) {
